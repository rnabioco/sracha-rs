...
pub(super) fn expand_via_page_map(
    decoded_ints: Vec<u8>,
...
pub(crate) fn decode_zip_encoding(decoded: &blob::DecodedBlob<'_>) -> Result<Vec<u8>> {
    let hdr_version = decoded.headers.first().map(|h| h.version).unwrap_or(0);
...
/// before.
pub(crate) fn decode_quality_encoding(decoded: &blob::DecodedBlob<'_>) -> Result<Vec<u8>> {
    if !decoded.data.is_empty()
